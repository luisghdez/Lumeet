"""
Lightweight JSON metadata store for raw hook videos uploaded to GCS.

Each hook record tracks a ``generated_raw.mp4`` that can be reused
for remixing with different captions, sounds, and extension videos.
"""
...
from config import HOOK_METADATA_FILE


class HookMetadataStore:
    ...
    def save(self, hook_id: str, payload: Dict[str, Any]) -> None:
        with self._lock:
            all_items = self._read_all_unlocked()
            all_items[hook_id] = payload
    ...
    def update(self, hook_id: str, **fields: Any) -> Optional[Dict[str, Any]]:
        ...
        with self._lock:
            all_items = self._read_all_unlocked()
            record = all_items.get(hook_id)
            if not isinstance(record, dict):
                return None
            record.update(fields)
            all_items[hook_id] = record
            self._write_all_unlocked(all_items)
            return record
...
hook_metadata_store = HookMetadataStore(HOOK_METADATA_FILE)
